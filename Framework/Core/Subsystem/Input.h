...
public:
	...
	const bool KeyDown(IN const KeyCode& key) const { return keyMap[static_cast<unsigned long>(key)] == static_cast<uint>(KeyStatus::KEY_INPUT_STATUS_DOWN); }
	const bool KeyUp(IN const KeyCode& key) const { return keyMap[static_cast<unsigned long>(key)] == static_cast<uint>(KeyStatus::KEY_INPUT_STATUS_UP); }
	const bool KeyPress(IN const KeyCode& key) const { return keyMap[static_cast<unsigned long>(key)] == static_cast<uint>(KeyStatus::KEY_INPUT_STATUS_PRESS); }
