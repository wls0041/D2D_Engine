...
{
	if (!scripting) {
		LOG_ERROR("ScriptModule : LoadScript - scripting is Empty");
		return false;
	}
	...
	if (result < 0) {
		LOG_ERROR("ScriptModule : GetAsIScriptModule - Failed to Start new Module");
		return false;
	}
	...
	if (result < 0) {
		LOG_FERROR("ScriptModule : GetAsIScriptModule - Failed to Load script \"%s\"", path);
		return false;
	}
	...
	if (result < 0) {
		LOG_FERROR("ScriptModule : GetAsIScriptModule - Failed to Compile script \"%s\"", FileSystem::GetFileNameFromFilePath(path));
		return false;
	}
...
{
	if (!builder) {
		LOG_ERROR("ScriptModule : GetAsIScriptModule - Builder is Empty");
		return nullptr;
	}
