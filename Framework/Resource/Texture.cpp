...
auto Texture::CreateShaderResourceView(const uint & width, const uint & height, const uint & channels, const DXGI_FORMAT & format, const std::vector<MipLevel>& mipChain) -> const bool
{
	...
	auto result = SUCCEEDED(graphics->GetDevice()->CreateTexture2D(&desc, subResourceDatas.data(), &texture));

	if (!result) {
		Log::Error("Texture::CreateShaderResourceView - Failed to create ID3D11Textuer2D");
		return false;
	}

	result = SUCCEEDED(graphics->GetDevice()->CreateShaderResourceView(texture, &srvDesc, &srv));

	if (!result) Log::Error("Texture::CreateShaderResourceView - Failed to create ID3D11ShaderResourceView");
...
auto Texture::CreateShaderResourceView(const uint & width, const uint & height, const uint & channels, const DXGI_FORMAT & format, const MipLevel & mipLevel, const bool & bGenerateMipChain) -> const bool
{
	...
	if (bGenerateMipChain) {
		if (width < 4 || height < 4) {
			Log::WriteFormatError("Texture::CreateShaderResourceView - MipChain is too small to generate as dimension %dx%d", width, height);
			return false;
		}
	}
	...
	subResourceData.SysMemSlicePitch = 0; //z축
	...
	ID3D11Texture2D *texture = nullptr;
	auto result = SUCCEEDED(graphics->GetDevice()->CreateTexture2D(&desc, bGenerateMipChain ? nullptr : &subResourceData, &texture)); //2번째 변수. 아무것도 안 넣으면 빈 텍스쳐가 나옴
	...
	if (!result) {
		if (bGenerateMipChain) {
			graphics->GetDeviceContext()->UpdateSubresource(texture, 0, nullptr, mipLevel.data(), width * channels * (bpc / 8), 0); //이 함수를 쓰기 위해 defualt로 지정함
			graphics->GetDeviceContext()->GenerateMips(srv);
		}
	}
	else Log::Error("Texture::CreateShaderResourceView - Failed to create ID3D11ShaderResourceView");
