...
/*
	...
	퀄리티에 따른 구분. 멀리있으면 bilinear, 가까이 있으면 lanczos
*/
...
ImageImporter::ImageImporter(Context * context) : context(context)
{
	...
	auto FreeImageErrorHandler = [](FREE_IMAGE_FORMAT fif, const char* message)
	{
		...
		Log::WriteFormatError("%s, Format : %s", text, format);
	};
...
auto ImageImporter::Load(const std::string & path, Texture * texture) -> const bool
{
	...
	{
		Log::WriteFormatError("ImageImporter::Load : Path \"%s\" is invalid.", path.c_str());
		return false;
	}
