...
void Widget_Inspector::ShowTilemap(Tilemap * tilemap)
{
	...
	static auto tileset_gird_color = Color(0, 1, 0, 1);
	static auto bTileset_grid = true;
	static auto bShowTileset = false;
	static auto w = tilemap->GetWidth();
	...
	if (ImGui::CollapsingHeader("Tilemap", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		ImGui::ColorEdit4("Color", tileset_gird_color);
				...
				if (ImGui::BeginTabItem(tileset->GetResourceName().c_str()))
				{
					...
					if (bTileset_grid)
					{
						...
						for (uint x = 0; x <= width; x += spacing)
							...
							(
								...
								ImVec2(x + windowPos.x, windowPos.y + height),
								tileset_gird_color
							);
						...
						for (uint y = 0; y <= height; y += spacing)
							...
							(
								...
								ImVec2(windowPos.x + width, y + windowPos.y),
								tileset_gird_color
							);
					}
					...
					if (ImGui::IsWindowHovered())
					{
						if (ImGui::IsMouseClicked(0))
						{
							...
							auto temp = ImVec2
							(
								mousePos.x + ImGui::GetScrollX(),
								mousePos.y + ImGui::GetScrollY()
							);

							uint x = static_cast<uint>(temp.x / spacing);
							uint y = static_cast<uint>(temp.y / spacing);

							TileData data;
							data.SpriteOffset = Vector2(static_cast<float>(x * spacing), static_cast<float>(y * spacing));
							data.SpriteSize = Vector2(static_cast<float>(spacing));
